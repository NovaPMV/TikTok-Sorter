...
Double-click **`Start ClipFinder.bat`**.
...
**When you're done for the day, close that black window to shut the app down.**
...
every video, which takes a while. A folder of a few hundred clips can take
...
**Updates after that are fast** — it only processes new or changed videos.
...
compared to your source files. Rough real-world figures for a **10-second
vertical TikTok**:
...
- **A package failed to install** → Install them one line at a time (Step 3), not
  all in one line. If one still fails, copy the error and ask in the Discord.
...
## A note on the videos you sort

This tool just helps you find clips faster in footage **you already have**. Please
respect the creators whose content you work with — credit them, and get their OK
where you can. It's the right thing to do and it protects you from takedowns and
strikes as you grow.
