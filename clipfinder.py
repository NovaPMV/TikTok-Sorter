...
def content_id(path: Path) -> str:
    ...
    This is what lets us recognize the SAME clip after it's been renamed or moved
    to a different folder, so we don't re-index or regenerate frames/previews.

    For speed we don't read the whole (large) video - we hash the file SIZE plus
    the first and last 1 MB. For real-world video files that combination is
    effectively unique; two different clips would have to share an exact size AND
    identical head+tail bytes to collide, which doesn't happen in practice.

    The frames folder and preview file are named from this id, so they travel
    with the file's content rather than its path."""
    ...
    try:
        with open(path, "rb") as f:
            ...
            if stat.st_size > CHUNK:
                # seek to the last CHUNK bytes (or start, for smallish files)
                f.seek(max(0, stat.st_size - CHUNK))
                h.update(f.read(CHUNK))
    except Exception:
        # If the file can't be read for some reason, fall back to size+mtime so
        # we still get a stable-ish id rather than crashing the whole index.
        h.update(f"{int(stat.st_mtime)}".encode())
    return h.hexdigest()[:16]                # 16 hex chars is plenty here


def extract_frames(video: Path, frames_dir: Path, cid: str) -> list[Path]:
    ...
    The output subfolder is named by CONTENT ID (cid), not the file path, so the
    frames stay valid even if the video is later renamed or moved."""
    # Each video gets its own subfolder of frames, named by content id, so
    # nothing collides and the frames survive the file being moved/renamed.
    out_dir = frames_dir / cid
...
def make_preview(video: Path, previews_dir: Path, cid: str) -> Path:
    ...
    The preview file is named by CONTENT ID (cid), not the file path, so it stays
    valid if the video is later renamed or moved. If a preview for this content
    already exists we reuse it instead of re-encoding."""
    previews_dir.mkdir(parents=True, exist_ok=True)
    out_path = previews_dir / f"{cid}.mp4"
    if out_path.exists() and out_path.stat().st_size > 0:
        return out_path                                 # already made; reuse it
...
def save_index():
    ...
    # Make sure the frames folder actually exists before writing into it. If the
    # user pointed the Frames box at a new/moved location that isn't there yet,
    # create it rather than crashing with "No such file or directory".
    try:
        meta_path.parent.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        # e.g. an invalid path, a drive letter that doesn't exist, or no
        # permission. Surface a clear, actionable message instead of a raw errno.
        raise RuntimeError(
            f"Can't write the index to the Frames folder '{meta_path.parent}'. "
...
def build_index_job():
    ...
    try:
        ...
        # PREFLIGHT: the index and generated data live in the Frames/Previews
        # folders. If you MOVED one of them and the box still points at the old
        # location (or you typed a path that doesn't exist / drive not connected),
        # bail out early with a clear message instead of a cryptic file error
        # partway through. We try to create them; if that fails, we explain why.
        for label, d in (("Frames", frames_dir), ("Previews", previews_dir)):
            if not str(d).strip():
                p["message"] = (f"No {label} folder set. Pick a {label} folder, "
                                f"click Save folders, then Build / update index.")
                return
            try:
                d.mkdir(parents=True, exist_ok=True)
            except Exception as e:
                p["message"] = (f"Can't use the {label} folder '{d}'. If you moved "
                                f"it, update the {label} box to its new location, "
                                f"click Save folders, then index again. ({e})")
                return
        ...
        #
        # We identify files by CONTENT ID (their bytes), not path, so a file that
        # was merely renamed or moved is recognized as already-indexed and gets
        # RE-POINTED to its new path instead of reprocessed.
        #
        # Build a map of the content ids we already have indexed -> their old key
        # (path), so we can detect moves.
        cid_to_oldkey = {}
        for k, info in STATE["videos"].items():
        ...
        for v, sf in all_videos:
            ...
            if known is not None and known.get("fingerprint") == fp:
                if known.get("source") != sf:
                    known["source"] = sf
                # UPGRADE: older indexes have no content_id. Backfill it now (cheap
                # - just reads a bit of the file) so future moves are recognized.
                # NOTE: the frames/preview for these older entries were named by
                # the old path-hash scheme; they keep working at their current path.
                # If such a file is later moved, it'll be reprocessed once (its old
                # frames can't be found by content id) and then be move-proof after.
                if not known.get("content_id"):
                    known["content_id"] = content_id(v)
                continue

            # Compute content id to check whether this is a moved/renamed known file.
            cid = content_id(v)
            old_key = cid_to_oldkey.get(cid)

            # Case 2: this content is already indexed under a DIFFERENT path
            # (a move/rename). Re-point the existing record to the new path -
            # no frames, embeddings, or preview regeneration needed.
            if old_key is not None and old_key != key and old_key in STATE["videos"]:
                rec = STATE["videos"].pop(old_key)
                rec["fingerprint"] = fp        # refresh (mtime may differ after a move)
                rec["source"] = sf
                rec["content_id"] = cid
                STATE["videos"][key] = rec
                # move its frame_records over to the new path key
                for fr in STATE["frame_records"]:
                    if fr["video"] == old_key:
                        fr["video"] = key
            ...
            # Case 3: same path but the file changed (re-encoded/replaced), OR a
            # genuinely new file. Either way it needs processing.
        ...
        skipped = len(all_videos) - total - repointed
        ...
        cancelled = False
        for i, (video, fp, sf, cid) in enumerate(to_process):
            ...
            # 1) frames (backend only), named by content id
            frames = extract_frames(video, frames_dir, cid)
            # 2) embeddings for those frames
            vecs = embed_images(frames)
            # 3) one preview for the UI, named by content id (reused if present)
            preview = make_preview(video, previews_dir, cid)
            ...
            STATE["videos"][str(video)] = {
                "preview": str(preview),
                "fingerprint": fp,
                "content_id": cid,     # location-independent identity (survives moves)
                "duration": duration,
...
def build_result_item(video: str, score=None):
    ...
    if not info:
        return None
    # Send the content id so the grid can ask for the preview from the CURRENT
    # previews folder (previews are named <content_id>.mp4). This is what makes
    # previews keep working after the previews folder is renamed or moved. For
    # older records without a content id, fall back to the stored preview path.
    return {
        "video": video,                          # original source path (for copying/opening)
        "cid": info.get("content_id", ""),       # preview is <cid>.mp4 in the previews folder
        "preview": info.get("preview", ""),      # legacy absolute path (fallback only)
...
@app.get("/preview")
def get_preview(path: str = "", cid: str = ""):
    # Preferred: serve by CONTENT ID from the CURRENT previews folder. Previews
    # are named <cid>.mp4, so this keeps working even after the previews folder
    # is renamed or moved (the stored absolute path would be stale, but the
    # current folder + cid always resolves to the right file).
    if cid:
        previews_dir = Path(STATE["folders"].get("previews", "") or ".")
        p = previews_dir / f"{cid}.mp4"
        if p.exists():
            return FileResponse(str(p), media_type="video/mp4")
        # fall through to the legacy path if the cid file isn't there
...
function renderPage(){
  ...
  for(const item of slice){
    ...
    // Prefer requesting by content id (served from the CURRENT previews folder,
    // so it survives that folder being renamed/moved); fall back to the stored
    // path for older records that predate content ids.
    vid.src = item.cid
      ? '/preview?cid=' + encodeURIComponent(item.cid)
      : '/preview?path=' + encodeURIComponent(item.preview);
