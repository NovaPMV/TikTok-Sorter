...
STATE = {
    ...
    "folders": {"source": "", "frames": "", "previews": "", "destination": ""},
...
def load_settings():
    """Read the four folder paths from the settings file, if it exists, and
    pre-fill them. Called once at startup."""
    try:
        if SETTINGS_FILENAME.exists():
            saved = json.loads(SETTINGS_FILENAME.read_text())
            for k in ("source", "frames", "previews", "destination"):
                if saved.get(k):
...
def build_index_job():
    ...
    try:
        source = Path(STATE["folders"]["source"])
        frames_dir = Path(STATE["folders"]["frames"])
        previews_dir = Path(STATE["folders"]["previews"])

        # Find every video file in the source folder (recursively).
        all_videos = [f for f in source.rglob("*") if f.suffix.lower() in VIDEO_EXTENSIONS]
        ...
        for v in all_videos:
            ...
            if (known is None) or (known.get("fingerprint") != fp):
                to_process.append((v, fp))

        # PRUNE: drop videos that no longer exist on disk.
        existing_keys = {str(v) for v in all_videos}
        ...
        cancelled = False
        for i, (video, fp) in enumerate(to_process):
...
def search(query: str, max_videos: int = 0):
    """Search all indexed frames for the query and return one result per video,
...
def browse_all():
    """Return every indexed video (no search), sorted by filename. This is the
...
async def set_folders(request: Request):
    ...
    for key in ("source", "frames", "previews", "destination"):
        if key in data:
...
@app.get("/browse_all")
def do_browse_all():
    """All indexed videos, no search - the default browse view."""
    return JSONResponse(browse_all())
...
<style>
  ...
  .tile{ position:relative; border:2px solid transparent; border-radius:8px;
    overflow:hidden; cursor:pointer; background:#0f1115; }
  ...
  /* Open File button sits at the right end of the info row */
  .openbtn{ margin-left:auto; background:#2a2e37; color:#e8e8ea; border:none;
...
<div class="row">
  ...
  <div><label>Per page</label>
    <input id="perpage" type="range" min="4" max="50" value="8"
           oninput="perpageLabel.textContent=this.value; renderPage()">
    <span id="perpageLabel">8</span></div>
...
async function saveFolders(){
  const body = {
    source: f_source.value, frames: f_frames.value,
...
async function pollProgress(){
  ...
  if (p.running){
    ...
    // Show all indexed videos now that indexing is done, so you can browse/copy
...
function renderPage(){
  ...
  for(const item of slice){
    const isCopied = copiedSet.has(item.video) || item.copied;
  ...
  // update the page-number box and the "/ N" total
  document.getElementById('pagenum').value = results.length ? (page+1) : 0;
  document.getElementById('pageinfo').textContent =
    '/ ' + (results.length ? pageCount() : 0);
  updateCount();
}
...
function selectAllOnPage(){
  const start = page*perPage();
  // skip clips already copied this session - they can't be selected
  results.slice(start, start+perPage())
    .filter(i => !(copiedSet.has(i.video) || i.copied))
...
function nextPage(){
  if(page < pageCount()-1){ page++; selected.clear(); renderPage(); }
}
function prevPage(){
  if(page > 0){ page--; selected.clear(); renderPage(); }
}
// Jump straight to a typed page number.
function gotoPage(){
  let n = parseInt(document.getElementById('pagenum').value);
  if(isNaN(n)) return;
